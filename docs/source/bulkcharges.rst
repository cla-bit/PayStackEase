...
.. py:class:: BulkChargesClientAPI()
    ...
    .. py:method:: fetch_charge_bulk_batch(id_or_code: str, page_values: Optional[PageModel] = None, date_values: Optional[DatePageModel] = None, status: Optional[STATUS] = None)→ PayStackResponse
    ...
    .. py:method:: initiate_bulk_charge(objects: Union[BulkChargeObject, List[BulkChargeObject]])→ PayStackResponse
    ...
    .. py:method:: list_bulk_charge_batches(page_values: Optional[PageModel] = None, date_values: Optional[DatePageModel] = None)→ PayStackResponse
...
.. code-block:: python
    ...
    >>> response = paystack_client.bulk_charges.initiate_bulk_charge(objects)  # pass it as a single BulkChargeObject instance
    >>> print(response)

    >>> response = paystack_client.bulk_charges.initiate_bulk_charge([objects, objects1])  # pass it as a list BulkChargeObject instances
    >>> print(response)
