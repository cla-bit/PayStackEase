...
def test_create_subscription(
    subscriptions_client, customer, plan_code, authorization, start_date
):
    """Test for synchronous Customers"""
...
def test_generate_update_subscription(subscriptions_client):
    ...
    response = subscriptions_client.generate_update_subscription(
...
def test_send_update_subscription(subscriptions_client):
    ...
    response = subscriptions_client.send_update_subscription_link(
        subscription_code=sub_code
    )
    assert len(responses.calls) == 1
    assert responses.calls[0].request.url == url
