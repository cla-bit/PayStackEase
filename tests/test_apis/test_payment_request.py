...
def test_create_payment_request(
    ...
    response = payment_requests_client.create_payment_request(
        customer=customer,
        amount=amount,
        draft=draft,
        has_invoice=has_invoice,
...
def test_list_payment_requests(
    ...
    response = payment_requests_client.list_payment_requests(
        per_page=per_page,
        page=page,
        from_date=from_date,
        to_date=to_date,
...
def test_send_notification(payment_requests_client):
    ...
    response = payment_requests_client.send_notification(code=payment_code)
    assert len(responses.calls) == 1
    assert responses.calls[0].request.url == url
    assert response is not None
...
def test_update_payment_request(
    ...
    response = payment_requests_client.update_payment_request(
...
def test_archive_request(payment_requests_client):
    ...
    response = payment_requests_client.archive_payment_request(code=payment_code)
    assert len(responses.calls) == 1
    assert responses.calls[0].request.url == url
    assert response is not None
