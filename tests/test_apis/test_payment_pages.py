...
@responses.activate
def test_create_payment_page(payment_pages_client, name, description, amount, split_code, page_slug, redirect_url, metadata, custom_fields):
    """ Test for synchronous Customers """
...
def test_check_slug(payment_pages_client, products):
    ...
    response = payment_pages_client.add_products(payment_id=payment_id, product=products)
    assert len(responses.calls) == 1
    assert responses.calls[0].request.url == url
    assert json.loads(responses.calls[0].request.body) == expected_data
    assert response is not None
