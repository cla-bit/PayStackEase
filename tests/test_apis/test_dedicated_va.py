...
def test_fetch_dedicated_account(dva_client):
    ...
    response = dva_client.fetch_dedicated_account(
...
def test_deactivate_dedicated_account(dva_client):
    ...
    response = dva_client.deactivate_dedicated_account(
        dedicated_account_id=dedicated_account_id
    )
    assert len(responses.calls) == 1
    assert responses.calls[0].request.url == url
...
@responses.activate
def test_requery_dva(
    dva_client, customer_id_code, subaccount, split_code, preferred_bank
):
    """Test for synchronous Customers"""
