...
async def test_create_customer(
    ...
    response = await async_customers_client.create_customer(
        email, first_name, last_name, phone, metadata
    )
...
async def test_validate_customer(
    async_customers_client,
    mocked_responses,
    first_name,
    last_name,
...
async def test_whitelist_blacklist_customer(
    async_customers_client, mocked_responses, email_or_code, risk_action
):
    """Test for syncjhronouus Customers"""
...
async def test_deactivate_authorization(
    async_customers_client, mocked_responses, authorization_code
):
    """Test for syncjhronouus Customers"""
    url = "https://api.paystack.co/customer/deactivate_authorization"
    expected_data = {"authorization_code": authorization_code}
    mocked_responses.post(url, status=200, payload=expected_data)
    response = await async_customers_client.deactivate_authorization(
...
async def test_update_customer(
    async_customers_client, mocked_responses, first_name, last_name, phone, metadata
):
    """Test for syncjhronouus Customers"""
    ...
    mocked_responses.put(url, status=200, payload=expected_data)
...
async def test_fetch_customer(async_customers_client, mocked_responses):
    ...
    mocked_responses.get(url, status=200, payload=expected_data)
...
async def test_list_customers(
    async_customers_client, mocked_responses, per_page, page, from_date, to_date
):
    """
    ...
    response = await async_customers_client.list_customers(
