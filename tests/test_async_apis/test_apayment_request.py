...
async def test_fetch_payment_request(async_payment_requests_client, mocked_responses):
    ...
    response = await async_payment_requests_client.fetch_payment_request(
        id_or_code=payment_id
    )
    mocked_responses.assert_called()
    assert response is not None
...
async def test_verify_payment(async_payment_requests_client, mocked_responses):
    ...
    response = await async_payment_requests_client.verify_payment_request(
        code=payment_code
    )
    mocked_responses.assert_called()
    assert response is not None
...
async def test_finalize_payment(
    async_payment_requests_client, mocked_responses, notice
):
    """Test for synchronous Customers"""
    payment_code = "test-payment-code"
    ...
    response = await async_payment_requests_client.finalize_payment_request(
...
async def test_update_payment_request(
    async_payment_requests_client,
    mocked_responses,
    customer,
    amount,
...
async def test_archive_request(async_payment_requests_client, mocked_responses):
    ...
    response = await async_payment_requests_client.archive_payment_request(
        code=payment_code
    )
    mocked_responses.assert_called()
    assert response is not None
