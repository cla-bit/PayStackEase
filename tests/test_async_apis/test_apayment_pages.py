...
async def test_create_payment_page(
    async_payment_pages_client,
    mocked_responses,
    name,
    description,
    amount,
...
async def test_list_pages(
    async_payment_pages_client, mocked_responses, from_date, to_date, per_page, page
):
    """Test for synchronous Customers"""
...
async def test_list_pages(async_payment_pages_client, mocked_responses):
    ...
    response = await async_payment_pages_client.fetch_payment_page(
        page_id_or_slug=id_or_slug
    )
    mocked_responses.assert_called()
    assert response is not None
...
async def test_update_page(
    async_payment_pages_client, mocked_responses, name, description, amount, active
):
    """Test for synchronous Customers"""
...
async def test_check_slug(async_payment_pages_client, mocked_responses):
    ...
    response = await async_payment_pages_client.check_slug_available(
        page_slug=id_or_slug
    )
    mocked_responses.assert_called()
    assert response is not None
...
async def test_check_slug(async_payment_pages_client, mocked_responses, products):
    ...
    response = await async_payment_pages_client.add_products(
