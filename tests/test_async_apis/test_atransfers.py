...
async def test_create_transfer(async_transfers_client, mocked_responses, transfer_source, amount, transfer_recipient, reason, currency, reference):
    """ Test for synchronous Customers """
...
async def test_finalize_transfer(async_transfers_client, mocked_responses, transfer_code, otp):
    """ Test for synchronous Customers """
    ...
    response = await async_transfers_client.finalize_transfer(transfer_code=transfer_code, otp=otp)
    mocked_responses.assert_called()
    assert response is not None
...
async def test_bulk_transfer(async_transfers_client, mocked_responses, transfer_source, transfers):
    """ Test for synchronous Customers """
    ...
    response = await async_transfers_client.initiate_bulk_transfer(transfer_source=transfer_source, transfers=transfers)
    mocked_responses.assert_called()
    assert response is not None
...
async def test_list_transfers(async_transfers_client, mocked_responses, customer_id, from_date, to_date, per_page, page):
    """ Test for synchronous Customers """
