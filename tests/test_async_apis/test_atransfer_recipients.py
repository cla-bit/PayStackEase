...
async def test_create_recipient(
    async_transfer_recipients_client,
    mocked_responses,
    recipient_type,
    recipient_name,
    account_number,
...
async def test_create_bulk_recipient(
    async_transfer_recipients_client, mocked_responses, batch
):
    """Test for synchronous Customers"""
    ...
    response = await async_transfer_recipients_client.bulk_create_transfer_recipient(
        batch=batch
    )
    mocked_responses.assert_called()
    assert response is not None
...
async def test_fetch_recipient(async_transfer_recipients_client, mocked_responses):
    ...
    response = await async_transfer_recipients_client.fetch_transfer_recipient(
...
async def test_update_recipient(
    async_transfer_recipients_client, mocked_responses, recipient_name, recipient_email
):
    """Test for synchronous Customers"""
    recipient_id = "test-recipient-id"
    url = f"https://api.paystack.co/transferrecipient/{recipient_id}"
    ...
    response = await async_transfer_recipients_client.update_transfer_recipient(
        id_or_code=recipient_id,
        recipient_name=recipient_name,
...
async def test_delete_recipient(async_transfer_recipients_client, mocked_responses):
    ...
    response = await async_transfer_recipients_client.delete_transfer_recipient(
        id_or_code=recipient_id
    )
    mocked_responses.assert_called()
    assert response is not None
